...
// MsgTutorial 파일 형식을 사용하여 msg라는 메시지를 선언
...
// 메시지에 사용될 변수 선언
	...
	while(ros::ok())
	{
		...
		msg.data = count;		// count 값을 msg 하위 data 메시지에 담는다.

		ROS_INFO("send_msg = %d", msg.stamp.sec);	// stamp.sec 표시
		ROS_INFO("send_msg = %d", msg.stamp.nsec);	// stamp.nsec 표시
		ROS_INFO("send_msg = %d", msg.data);		// data 표시

		ros_tutorial_pub.publish(msg);			// 메시지를 발행한다

		loop_rate.sleep();				// 위에서 정한 루프 주기에 따라 슬립한다.
